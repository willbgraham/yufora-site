...
import { setArticleStatus } from "@/app/actions/news";
import type { ProductStatus } from "@/lib/db/schema";
import { Button } from "@/components/ui/Button";

export default function ArticleStatusControls({
  articleId,
  status,
}: {
  articleId: string;
  ...
  const set = (next: ProductStatus) =>
    startTransition(() => setArticleStatus(articleId, next));
  ...
  return (
    <div className="flex flex-wrap items-center gap-3">
      ...
      <button
        ...
        disabled={pending}
        onClick={() => {
          if (confirm("Archive this article? It's removed from the news page and this list.")) {
            set("archived");
          }
        }}
